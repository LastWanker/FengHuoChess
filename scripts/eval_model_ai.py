"""Evaluate model AI against baseline/weak opponents in headless EVE."""
...
def _resolve_paths(args: argparse.Namespace) -> dict[str, str]:
    ...
    default_black_fast = _first_existing(
        [
            repo_root / "artifacts/models/model_tiny_policy_fast_best_black.pt",
...
def _validate_model_paths(paths: dict[str, str]) -> None:
    ...
    if not any(chosen):
        raise ValueError(
            ...
            "or place best checkpoints under artifacts/models."
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate ModelAISource in headless EVE")
    ...
    parser.add_argument("--game-mode", choices=["fast", "slow"], default="slow", help="Initial game mode")
    parser.add_argument("--games", type=int, default=30, help="Games per color assignment")
    parser.add_argument("--sample-top-k", type=int, default=2, help="Model softmax sample top-k (0 disables sampling)")
    parser.add_argument("--sample-temperature", type=float, default=1.0, help="Model softmax sample temperature")
    parser.add_argument(
        "--explore-second-prob",
        type=float,
        default=0.0,
        help="Legacy top2 exploration probability (keep 0 for softmax-only)",
    )
    parser.add_argument(
        "--explore-gap-threshold",
        type=float,
        default=0.0,
        help="Legacy top2 exploration gap threshold",
    )
    ...
    if args.verbose:
        _print_round(f"model(black) vs {args.opponent}(white)", s1)
        _print_round(f"{args.opponent}(black) vs model(white)", s2)
    ...
    print(
        ...
        f"explore_second_prob={args.explore_second_prob:.3f} explore_gap_threshold={args.explore_gap_threshold:.3f}"
    )
    print(
        " ".join(
            [
