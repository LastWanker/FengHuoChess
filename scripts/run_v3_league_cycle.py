"""Run V3 model-selfplay league cycles with robust pairwise promotion checks."""
...
def _format_opponent_pool(pool: dict[str, float]) -> str:
    ordered = ["random", "v2_best", "baseline", "old_best"]
    chunks: list[str] = []
    for key in ordered:
        val = float(pool.get(key, 0.0))
        if val > 0.0:
            label = "v2best" if key == "v2_best" else ("oldbest" if key == "old_best" else key)
...
def _run_duel_and_promote(
    *,
    ...
    v3_decision_top_k: int,
    v3_decision_alpha: float,
    v3_decision_beta: float,
    v3_max_window: int,
    round_no: int,
    ...
    best_spec = ArenaSourceSpec(
        ...
        decision_top_k=max(1, int(v3_decision_top_k)),
        decision_alpha=float(v3_decision_alpha),
        decision_beta=float(v3_decision_beta),
        max_window=max(2, int(v3_max_window)),
    )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Run V3 model-selfplay league cycles with robust pairwise promotion")
    parser.add_argument("--rounds", type=int, default=8, help="Number of consecutive cycles to run")
    ...
    parser.add_argument("--pool-games", type=int, default=2400, help="Model selfplay games per round")
    ...
    parser.add_argument(
        "--pool-opponent-pool",
        default="random:0.2,v2best:0.4,baseline:0.35,oldbest:0.05",
    ...
    parser.add_argument(
        ...
        default="random:0.05,v2best:0.45,baseline:0.4,oldbest:0.1",
    ...
    parser.add_argument("--split-a", type=int, default=10, help="Length split A for curation")
    parser.add_argument("--split-b", type=int, default=18, help="Length split B for curation")
    ...
    parser.add_argument("--arena-games", type=int, default=240, help="Duel games per color assignment")
    ...
    parser.add_argument("--v3-decision-top-k", type=int, default=5, help="Value re-rank candidate top-k")
    parser.add_argument("--v3-decision-alpha", type=float, default=1.0, help="Policy prior coefficient")
    parser.add_argument("--v3-decision-beta", type=float, default=0.5, help="Value coefficient")
    parser.add_argument("--v3-max-window", type=int, default=12, help="Temporal window length for V3 inference")
    parser.add_argument("--v3-train-window", type=int, default=12, help="Temporal window length for V3 training")
    parser.add_argument(
        "--best-model-black",
    ...
    parser.add_argument(
        ...
        help="V2 black slow model path for v2_best opponent bucket",
    )
    parser.add_argument(
        "--v2-model-white-slow",
        default="artifacts/value_plus_models/model_tiny_policy_slow_best_white.pt",
        help="V2 white slow model path for v2_best opponent bucket",
    )
    parser.add_argument(
        "--v2-model-black-fast",
        default="",
        help="Optional V2 black fast model path (defaults to --v2-model-black-slow)",
    )
    parser.add_argument(
        "--v2-model-white-fast",
        default="",
        help="Optional V2 white fast model path (defaults to --v2-model-white-slow)",
    )
    parser.add_argument(
        "--candidate-model-black",
        default="artifacts/v3_transformer/models/model_v3_candidate_black.pt",
        help="Candidate black model output path",
    )
    parser.add_argument(
        "--candidate-model-white",
        default="artifacts/v3_transformer/models/model_v3_candidate_white.pt",
    ...
    parser.add_argument(
        ...
        default="artifacts/v3_transformer/datasets/model_pool_balanced_trace.jsonl",
        help="Legacy alias (V3 uses shared balanced trace)",
    )
    parser.add_argument(
    ...
    if args.v3_decision_top_k <= 0:
        raise SystemExit("--v3-decision-top-k must be > 0")
    if args.v3_max_window < 2:
        raise SystemExit("--v3-max-window must be >= 2")
    ...
    v2_model_black_slow = repo_root / args.v2_model_black_slow
    v2_model_white_slow = repo_root / args.v2_model_white_slow
    ...
    v2_model_black_fast = args.v2_model_black_fast.strip() or args.v2_model_black_slow
    v2_model_white_fast = args.v2_model_white_fast.strip() or args.v2_model_white_slow
    ...
    value_plus_dir = repo_root / "artifacts" / "value_plus_models"
    value_plus_best_black = value_plus_dir / "model_tiny_policy_slow_best_black.pt"
    value_plus_best_white = value_plus_dir / "model_tiny_policy_slow_best_white.pt"
    value_plus_shared_best = value_plus_dir / "model_tiny_policy_slow_best.pt"
    value_plus_init_model = value_plus_dir / "model_tiny_policy_slow.pt"
    ...
    _copy_if_missing(
        ...
        [
            ...
            value_plus_best_black,
            value_plus_best_white,
            legacy_init_model,
            legacy_shared_best,
            legacy_best_black,
            legacy_best_white,
    ...
    for round_idx in range(args.rounds):
        ...
        print(
            _c_info(
                ...
                f"split={args.split_a}/{args.split_b} "
                ...
                f"v3_window(train/infer)={args.v3_train_window}/{args.v3_max_window} "
                f"v3_decision(topk/alpha/beta)="
                f"{args.v3_decision_top_k}/{args.v3_decision_alpha:.2f}/{args.v3_decision_beta:.2f} "
        ...
        _run_step(
            ...
            [
                ...
                "--v2-model-black-slow",
                args.v2_model_black_slow,
                "--v2-model-white-slow",
                args.v2_model_white_slow,
                "--v2-model-black-fast",
                v2_model_black_fast,
                "--v2-model-white-fast",
                v2_model_white_fast,
                ...
                "--decision-top-k",
                str(args.v3_decision_top_k),
                "--decision-alpha",
                str(args.v3_decision_alpha),
                "--decision-beta",
                str(args.v3_decision_beta),
                "--max-window",
                str(args.v3_max_window),
                "--pool-mode",
                args.pool_mode,
                "--opponent-pool",
                round_pool_text,
        ...
        result = _run_duel_and_promote(
            ...
            v3_decision_top_k=args.v3_decision_top_k,
            v3_decision_alpha=args.v3_decision_alpha,
            v3_decision_beta=args.v3_decision_beta,
            v3_max_window=args.v3_max_window,
            round_no=round_no,
