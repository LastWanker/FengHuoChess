"""Parallel exporter for model selfplay traces (mirror or mixed opponent pool)."""
...
def _run_chunk(
    *,
    ...
    teacher_source = MasterAISource(rng=random.Random(chunk_seed + 53))
    baseline_source = HeuristicAISource(rng=random.Random(chunk_seed + 67))

    normalized_pool = dict(opponent_pool)
    opponent_counts = {"random": 0, "teacher": 0, "baseline": 0, "old_best": 0}
    ...
    for offset in range(num_games):
        ...
        _seed_source(teacher_source, seed + 53)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Parallel model selfplay export (mirror or mixed pool)")
    ...
    parser.add_argument(
        ...
        help="Used when --pool-mode=mixed; keys: random, teacherAI, baseline, oldbest",
    )
    parser.add_argument(
        "--sample-top-k",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Legacy top2 exploration probability (typically keep 0 with top-k softmax)",
    )
    parser.add_argument(
    ...
    print("--- Parallel Model Selfplay Export ---")
    print(
        ...
        f"sample_temperature={args.sample_temperature:.2f} pool_mode={args.pool_mode} start_game_id={game_id_offset}"
    )
                ...
                for future in done_now:
                    ...
                    _render_progress(
                        ...
                        extra=(
                            ...
                            f"pool={opponent_counts['random']}/{opponent_counts['teacher']}/"
